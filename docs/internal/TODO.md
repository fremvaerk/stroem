...
- [ ] **`claim_terminal_handling` returns `false` on a transient CAS DB error → terminal handling silently dropped** behind one `warn!` (found in Task 4 re-review of the creation-settlement unification plan, 2026-09-07). Add a `_server` log line on the job so the drop is visible in the job's own log view, not just server logs.
