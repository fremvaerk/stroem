...
- [ ] **Tera render errors can leak secret values into persisted step errors** — `render_template` returns Tera's error verbatim, and Tera's `try_get_value!` embeds the offending value in the message: `{{ secret.API_KEY | round }}` yields ``Filter `round` was called on an incorrect value: got "SUPER-SECRET-7f3a" but expected a f64`` (verified empirically 2026-09-11). Every claim-time render failure formats that chain with `format!("{:#}", e)` and hands it to `fail_claimed_step`, which persists it to `job_step.error_message` + `retry_history`, writes it to the job log via `append_server_log`, and returns it in the 422 body. `redact_response` (`web/api/jobs.rs`) masks `error_message` but NOT `retry_history`, which `GET /api/jobs/{id}` returns. Affects all four claim-time render paths (step input, action spec, image, agent prompts — `web/worker_api/jobs.rs`). Fix at the choke point: `fail_claimed_step` is the single call site for all four, so scrub the owner+caller workspace secret values from `error_msg` there before persisting (reuse `workspace_set::collect_redaction_values`).
...
- [ ] **4 · One owner for the render context** (Strong — top recommendation) — `web/worker_api/rendering.rs:68-122,233-268,366-397`, `job_creator.rs:601-663`, `web/worker_api/jobs.rs:740`. "What can an author reference here" has four answers and no owner; see the divergence defect above. 53 unit tests sit on the four builders, yet the bug that shipped was in the wiring between them (`crates/stroem-server/tests/integration_test.rs:2670` — "`claim_job` used to pass `owner_config.as_deref()`…" — needed a testcontainer to catch a one-argument mistake). CLAUDE.md's "deferred" cross-workspace agent-prompt gap *is* the fourth builder never learning about `owner_config`. Deepen: one module builds the step render context; which input, whose secrets, loop-or-not and which statuses become data it is given. Supersedes the existing Architecture item "Move `build_step_render_context` out of `job_creator.rs`". Was #7 of the 2026-09-08 review and still open.
