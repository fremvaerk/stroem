...
- [ ] **Task-level retry never fires in production** (found 2026-09-07): `TaskDef.retry` is validated and documented, but no writer sets `job.max_retries` from `task.retry.max_attempts` at job creation, so `orchestrate_after_step`'s task-level check sees NULL and `try_retry_job` is never reached. Integration tests only pass because they set the column with raw SQL (`UPDATE job SET max_retries = 1`). Fix: persist `max_attempts - 1` into `job.max_retries` in `create_job_for_task_inner` (and carry it through `set_retry_link` for the chain) + an integration test through the real creation path.
