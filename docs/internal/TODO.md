...
- [x] **`worker_token` credential reuse for Prometheus scrapes** (security, conf 80) — resolved: added `MetricsConfig.token: Option<String>`; middleware prefers it when set, falls back to `worker_token`. Tests: `metrics_token_overrides_worker_token` + auth edge-case tests.
