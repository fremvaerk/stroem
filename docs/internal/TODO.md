...
- [ ] **Jobs that settle at creation skip terminal handling** (no hooks, log archive, metrics) — creator marks terminal directly (`job_creator.rs:414-428`), only worker `complete_job` and cancellation call `handle_job_terminal`. Also the creator's check ignores `continue_on_failure`/`cancelled` and drops output vs `orchestrator.rs:132-203`. Fix planned in restart spec §4.3 (shared `settle_if_all_terminal` + `terminal_at_creation`). Spec §13/P3-P4.
- [ ] **Child `type: task` job that settles synchronously at creation never propagates to the parent step** (parent stays `running`) — `job_creator.rs:616-654` logs the child id only; propagation lives in terminal handling. Spec §13/P5.
- [ ] **`rerun` jobs are not top-level for workspace hooks** (`hooks.rs:95-98` lists api/user/trigger/webhook/mcp/retry) — a re-run's failure fires task-level hooks only. Spec §13/P6.
...
- [ ] **Post-commit job initialisation errors return 500 and leave a committed `pending` job** (`job_creator.rs:352-401`). Spec §13/P8.
