...
- [ ] **WebSocket backfill-to-live gap** — `ws.rs` reads and sends the backfill BEFORE subscribing to the broadcast channel, so chunks pushed in between never reach that connection. Pre-existing; the bounded-tail backfill (spec 2026-09-22-log-tail-streaming) neither widens nor closes it.
