...
## Bugs Found & Fixed
- [x] **Every `for_each` instance on a pod runner failed to start** (prod jobs `64cbab91`, `1b1c30e6`, 2026-09-23/24, `ai_traffic_model/backfill` step `ml-impressions[0]`). KubeRunner put the raw step name into the `stroem.io/step` label; `[`/`]` are not allowed in a Kubernetes label value, so the API server rejected the pod with a 422 before it existed — on every run since KubeRunner was added (the label predates `for_each`). `continue_on_failure: true` on the step hid it: the job stayed green and the downstream merge ran without the step's output. Fixed: `KubeRunner::step_labels` runs every label value through `sanitize_label_value` (`[-A-Za-z0-9_.]`, ≤ 63 chars, alphanumeric ends; already-valid values unchanged) and the raw name goes into the `stroem.io/step-name` annotation. Same fix: a pod name longer than 63 chars was cut at the end — where the instance index lives — so `step[10]` and `step[11]` of a long-named loop collided; `pod_name` now ends a truncated name in an FNV-1a hash of the untruncated inputs. Regression tests: `test_step_labels_for_each_instance_are_valid`, `test_pod_name_for_each_instances_with_long_names_stay_distinct` (`kubernetes.rs`).
