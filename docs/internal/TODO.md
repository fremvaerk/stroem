...
- [ ] **Task-level retry only runs in `orchestrate_after_step`**; a root job whose last live step is a `type: task` child reaches terminal via `propagate_to_parent` and never gets a retry job (pre-existing, found in Task 4 re-review of the creation-settlement unification plan, 2026-09-07).
