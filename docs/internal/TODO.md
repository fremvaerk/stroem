...
## Bugs Found & Fixed
- [ ] **`for_each` placeholder directly downstream of a FAILED step is never skipped → job stuck `running`** (found by Codex review of the restart spec, 2026-09-07). `skip_unreachable_steps` ignores placeholders (`job_step.rs:819-822`) and `expand_for_each_steps` just `continue`s when a dep is failed/cancelled (`job_creator.rs:722-737`); the 2026-09-02 fix covered only the *skipped*-dep case. Fix: in `expand_for_each_steps`, mark the placeholder `skipped` when a dep is failed/cancelled and the flow step is not `continue_on_failure`. Hotfix candidate. Spec §13/P1.
- [ ] **`type: agent` steps cannot be claimed by any worker** — `claim_ready_step` and the unmatched-step sweep exclude `action_type = 'agent'` (`job_step.rs:319`, `:967`). Regression from retry commit `ed67c76` (2026-04-08), which re-added `'agent'` to the `NOT IN` list after agent dispatch had moved worker-side; no test claims an agent step and prod has never run one, so it went unnoticed. Fix: drop `'agent'` from both lists + an integration test that claims an agent step with the `agent` capability. Spec §13/P2.
