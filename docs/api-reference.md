...
Webhook endpoints are served at `/hooks/` (outside `/api/`) and do **not** require user authentication. Authentication is handled per-trigger via an optional `secret` field in the webhook trigger definition.
...
Fires a webhook trigger by name, creating a new job for the associated task. If multiple workspaces define a webhook with the same name, the first match wins.

**Path parameters:**
- `name` -- Webhook trigger name (as defined in `triggers` YAML, e.g., `github-push`)

**Authentication:**

If the webhook trigger has a `secret` configured, the caller must provide it via one of:
...
The webhook handler builds a structured input object that is passed to the task:
...
| Field | Type | Description |
|-------|------|-------------|
| `body` | object/string/null | Parsed JSON (if `Content-Type: application/json`), raw string (other types), or `null` (GET requests) |
| `headers` | object | All request headers with lowercase keys |
| `method` | string | HTTP method (`"GET"` or `"POST"`) |
| `query` | object | Query parameters (excluding `secret`) |

Any `input` defaults defined in the trigger YAML are merged at the top level. Reserved keys (`body`, `headers`, `method`, `query`) are not overwritten by defaults.
...
- `"webhook"` -- Job created by a webhook call (source_id contains `"{workspace}/{trigger_key}"`, e.g. `"default/on-push"`)
