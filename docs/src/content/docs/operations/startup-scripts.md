...
**`type: script`** (with `runner: pod` or `runner: docker`): Startup scripts ARE mounted and run before the main process. The runner environment is set up with installed tools, configured auth, etc. This is the typical case for script execution.
