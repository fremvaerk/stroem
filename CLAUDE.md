...
- **Errors**: unresolvable action reference — unknown workspace, or a known workspace with no such action (`web/api/tasks.rs::classify_execute_error` matches `"has no action"`) → `BadRequest` (400), never 500 — same fix applied independently to missing/misnamed local connection references. Owner workspace being transiently unavailable (load/health condition, not a caller mistake) still surfaces as 500. The classifier matches on the FULL error context chain (`{:#}`), not just the outermost wrapper (a resolution error is nested several `.context()` layers deep) — `"is not available"` (configured-but-unloaded workspace) is excluded from the user-error match and always stays 500.
