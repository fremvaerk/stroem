...
- **Task-level retry is currently non-functional in production**: no creation path writes `job.max_retries` from `task.retry.max_attempts`, so `orchestrate_after_step`'s task-level check always sees NULL and `try_retry_job` is never reached — the existing tests only pass because they set the column with raw SQL (`UPDATE job SET max_retries = …`). Tracked in `docs/internal/TODO.md`; step-level retry is unaffected.
...
- Job-completion counter is incremented inside `claim_terminal_handling(state, job)` (`job_recovery.rs`), which is ALSO the exactly-once gate for every terminal side effect (propagation, task-retry creation, hooks, notify, log archive) via a `metrics_recorded_at` CAS. Any new terminal path must call it first and only act when it returns `true`. It takes `&AppState` (not a bare pool) so a fail-closed CAS error can be written to the job's own log view as `[orchestration] terminal handling claim failed: …` as well as logged at `error!`.
...
- **Task-tool children must not be born terminal**: a tool result reaches the agent step only through normal step completion (`propagate_to_parent`'s `agent_tool` branch), so a child that is already terminal at creation (every root step skipped by `when`, or a server-dispatched root step that failed) can never deliver one. `agent_task_tool` uses `create_child_job_for_task_detailed` and returns a 500 when `terminal_at_creation` is set. Do NOT "fix" this by calling `finalize_created_job` there — the worker has not yet recorded the child id in `agent_state`, so propagation would mark the agent step ready or terminal out from under the still-running worker. `agent_task_tool` first runs `reconcile_settled_children` on the CHILD's own subtree, so a child that settles because a nested grandchild settled is caught by the same check.
- **Agent registration barrier**: `propagate_to_parent`'s `agent_tool` branch returns `Ok(())` without touching the step when the parent step's `agent_state` is `None`, or when it parses but does not list this child in `pending_tool_calls`. The worker records pending child ids only AFTER the creation response, so anything earlier is unregistered — falling through to ordinary `mark_completed` would settle an agent step whose worker is still running. `agent_save_state` and `agent_suspend_step` (worker API) replay `propagate_to_parent` for every pending child that is already terminal right after persisting the state, which is what lifts the barrier. The child's terminal-handling claim was consumed by its own settlement, so the replay calls `propagate_to_parent` directly, not `handle_job_terminal`.
...
- **Settlement is shared**: `orchestrator::settle_if_all_terminal(pool, job_id, task)` is the only place *execution settlement* is decided (orchestrator AND job creation). Explicit terminal writes remain and are deliberate: cancellation (`cancellation.rs`), creation-time compensation (`job_creator.rs` init-error path), worker `complete_job`, and the scheduler's skipped-trigger rows. Rules: any untolerated `failed` → `failed`; else any `cancelled` → `cancelled`; else `completed` with aggregated terminal-step output. `create_job_for_task_detailed` returns `CreatedJob { job_id, terminal_at_creation }`; every HTTP/MCP/scheduler/webhook/hook/event-source/retry entry point calls `job_recovery::finalize_created_job` afterwards (runs `handle_job_terminal` once when terminal at creation + `reconcile_settled_children`). A job that settles inside creation is picked up by `reconcile_settled_children` (also called after every `handle_task_steps`), which walks the WHOLE descendant chain via `JobRepo::get_settled_descendants_with_running_parent_step` (recursive CTE over `parent_job_id`, capped at `MAX_TASK_DEPTH` = 10, rows deepest-first) — a direct-children-only query would miss a grandchild that settles at creation and leave the intermediate job stuck `running` forever. Scoped to `source_type = 'task'`: an `agent_tool` child that would be born terminal is rejected outright by `agent_task_tool` (see Agent Actions), never reconciled. Also requires **execution quiescence** — a terminal descendant with a `running` or `claimed` step of its own is skipped, so a cancelled child keeps its cancellation signal (and goes on collecting log lines) until its worker acknowledges by settling the step.
- **Drain before claiming**: all three convergence sites (`orchestrate_after_step`'s terminal block, `propagate_to_parent`'s parent-terminal block, `handle_job_terminal`) call `drained_for_terminal_handling` BEFORE `claim_terminal_handling` and skip the whole block while `JobStepRepo::has_live_steps` reports a `running`/`claimed` step. A job row can be terminal while its workers run — `JobRepo::cancel` stamps `cancelled` immediately — and without the gate the first worker to report would win the one-shot claim and `close_log` + archive while a sibling is still emitting; the later completion loses the claim and the archive is never refreshed. `clear_cancelled` sits behind the gate for the same reason: the cancellation signal must stay visible until the workers acknowledge. The drain check fails OPEN on a query error (an early archive still has a complete local-JSONL fallback; deferring on the last completion would drop the job's hooks outright). Regression test: `test_terminal_handling_waits_for_live_steps_to_drain`.
- Post-commit initialisation errors fail the live steps with `[creation] initialisation failed: …` and mark the job failed, both in ONE transaction (a half-applied compensation leaves failed steps under a non-terminal job that no sweep revisits). The coordinated `init` block covers root-step promotion/expansion, `type: task` dispatch, `type: approval` dispatch, and settlement — approval dispatch especially, because an approval step left `ready` is invisible to workers AND to the unmatched-step sweep. A DB outage during compensation itself still surfaces as a 500.
