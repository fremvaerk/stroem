...
- **Errors**: unresolvable action reference — unknown workspace, or a known workspace with no such action (`web/api/tasks.rs::is_user_error` matches `"has no action"`) → `BadRequest` (400), never 500 — same fix applied independently to missing/misnamed local connection references. Owner workspace being transiently unavailable (load/health condition, not a caller mistake) still surfaces as 500.
