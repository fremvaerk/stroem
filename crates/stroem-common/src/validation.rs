    ...
    for (trigger_name, trigger) in &config.triggers {
        ...
        match trigger {
            ...
            crate::models::workflow::TriggerDef::EventSource {
                ...
                } else if !config.tasks.contains_key(task) {
                    bail!(
                        "Trigger '{}' references non-existent consumer task '{}'",
                        trigger_name,
                        task
                    );
                }
                ...
                if target_task.contains('.') {
                    // Library reference — skip validation
                } else if !config.tasks.contains_key(target_task) {
                    bail!(
                        "Trigger '{}' references non-existent target task '{}'",
                        trigger_name,
                        target_task
                    );
                }

                // Consumer and target must be different
                if task == target_task {
                    bail!(
                        "Trigger '{}': consumer task and target_task cannot be the same ('{}')",
                        trigger_name,
                        task
                    );
                }
                ...
                if *backoff_secs < 1 {
                    bail!(
                        "Trigger '{}' has backoff_secs < 1 (got {})",
                        trigger_name,
                        backoff_secs
                    );
                }
                ...
                if let Some(mif) = max_in_flight {
                    if *mif < 1 {
                        bail!(
                            "Trigger '{}' has max_in_flight < 1 (got {})",
                            trigger_name,
                            mif
                        );
                    }
...
fn validate_connections(config: &WorkspaceConfig) -> Result<Vec<String>> {
    let mut warnings = Vec::new();
...
triggers:
  bad:
    ...
    task: test
"#;
        let result: Result<crate::models::workflow::WorkspaceConfig, _> =
...
triggers:
  on-push:
    ...
    task: ci-pipeline
"#;
        let result: Result<WorkspaceConfig, _> = serde_yaml::from_str(yaml);
        assert!(result.is_err());
        let err = result.unwrap_err().to_string();
        assert!(
            err.contains("name"),
...
"#,
    ...
    /// Wrap a single action into a minimal WorkspaceConfig.
...
"#;
    ...
    fn make_event_source_trigger_config(trigger_extra: &str) -> String {
        format!(
            r#"
actions:
  consumer-action:
    type: script
    script: "while true; do echo test; sleep 1; done"
tasks:
  consumer-task:
    flow:
      run:
...
"#,
    ...
    #[test]
    fn test_event_source_valid_basic() {
...
"#;
        ...
        let err = result.unwrap_err().to_string();
        assert!(
            err.contains("non-existent consumer task"),
            "Unexpected error: {}",
            err
        );
    }
    ...
    #[test]
    fn test_event_source_target_task_nonexistent() {
        let yaml = r#"
actions:
  consumer-action:
    type: script
...
"#;
        ...
        let err = result.unwrap_err().to_string();
        assert!(
            err.contains("non-existent target task"),
            "Unexpected error: {}",
            err
        );
    }
    ...
    #[test]
    fn test_event_source_consumer_equals_target_fails() {
        let yaml = r#"
actions:
  consumer-action:
    type: script
...
"#;
        ...
        let err = result.unwrap_err().to_string();
        assert!(
            err.contains("cannot be the same"),
            "Unexpected error: {}",
            err
        );
    }
    ...
    #[test]
    fn test_event_source_library_consumer_task_skipped() {
        // A consumer task with a dot is a library reference — validation is skipped.
        let yaml = r#"
actions:
  consumer-action:
    type: script
...
"#;
    ...
    #[test]
    fn test_event_source_library_target_task_skipped() {
        // A target task with a dot is a library reference — validation is skipped.
        let yaml = r#"
actions:
  consumer-action:
    type: script
...
"#;
    ...
    fn test_event_source_zero_backoff_secs() {
        let yaml = make_event_source_trigger_config("    backoff_secs: 0");
        let config: WorkspaceConfig = serde_yaml::from_str(&yaml).unwrap();
        let result = validate_workflow_config(&config);
        assert!(result.is_err());
        let err = result.unwrap_err().to_string();
    ...
    fn test_event_source_zero_max_in_flight() {
        let yaml = make_event_source_trigger_config("    max_in_flight: 0");
        let config: WorkspaceConfig = serde_yaml::from_str(&yaml).unwrap();
        let result = validate_workflow_config(&config);
        assert!(result.is_err());
        let err = result.unwrap_err().to_string();
    ...
    fn test_event_source_nonzero_max_in_flight_valid() {
        let yaml = make_event_source_trigger_config("    max_in_flight: 10");
        let config: WorkspaceConfig = serde_yaml::from_str(&yaml).unwrap();
        let result = validate_workflow_config(&config);
        assert!(result.is_ok(), "Expected Ok, got: {:?}", result);
    }
