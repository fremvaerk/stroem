...
pub async fn run_worker(
    ...
    let worker_id = {
        ...
        loop {
            tokio::select! {
                result = client.register(&config.worker_name, &config.capabilities, &config.tags, config.exclusive, Some(env!("CARGO_PKG_VERSION"))) => {
                    match result {
                        Ok(id) => break id,
                        Err(e) => {
                            attempt += 1;
    ...
    loop {
        ...
        let claim_result = tokio::select! {
            result = client.claim_step(worker_id, &config.capabilities, &config.tags, config.exclusive) => result,
            () = cancel_token.cancelled() => {
                tracing::info!("Shutdown requested, stopping poll loop");
                drop(permit);
                break;
    ...
    mod execute_step_tests {
        ...
        fn make_step(job_id: Uuid, cmd: &str) -> ClaimedStep {
            ClaimedStep {
                ...
                action_image: None,
                action_spec: Some(stroem_common::secret::Secret::new(
